...
const showItems = () => {
    ...
    if (foodLists.length > 0) {
        foodLists.forEach((item, key) => {
            ...
            newItem.innerHTML = `
            <div class="h-72 border-red-300 border-2 shadow-md rounded-lg bg-slate-100 hover:bg-sky-100 p-4 flex flex-col justify-center items-center">
            ...
            <button class="addCart w-full text-white bg-red-500 hover:bg-slate-700 text-base font-medium p-2 rounded-lg" onclick="addToCart(${key})">Add To Cart</button>
            </div>
            `;
...
const cartItems = () => {
    ...
    cartLists.forEach((value, key) => {
        ...
        if (value != null) {
            ...
            newDiv.innerHTML = `
                <div
                    class="my-2 border-white border-2 rounded-lg p-2 font-semibold text-black flex items-center justify-evenly gap-2">
                    ...
                    <div  class="quantity flex gap-3">
                        <button onclick ="changeQuantity(${key}, ${value.quantity - 1})" class="minus flex items-center justify-center bg-white w-5 h-5 p-1 rounded-xl">
                            - </button>
                                <div class="count">${value.quantity}</div>
                                <button  onclick ="changeQuantity(${key}, ${value.quantity + 1})" class="plus flex items-center justify-center bg-white w-5 h-5 p-1 rounded-xl"> +
                                </button>
                    </div>
                    <button onclick="deleteItem(${key})"><svg class="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
